...
pipeline {
  ...
  stages {
    ...
    stage("Terraform workspace") {
      steps {
       sh "terraform workspace new ${params.Name} ./terraform/aws"
      }
    }
    ...
    stage("Terraform apply") {
      steps {
	      withCredentials([string(credentialsId: "${params.AWS_Secret}", variable: 'AWS_SECRET'), string(credentialsId: "${params.AWS_Key}", variable: 'AWS_KEY')]) {
	        sh "terraform apply -auto-approve -var access_key=${AWS_Key} -var lidop_name=${params.Name} -var secret_key=${AWS_Secret} -var user_name=${params.User} -var password=${params.Password} ./terraform/aws"
        }      
      }
    }
